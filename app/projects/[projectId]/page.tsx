...
function ProjectPage({ params }: ProjectPageProps) {
  ...
  const handleDocumentUpload = async (files: File[]) => {
    ...
    const uploadPromises = files.map(async (file) => {
      try {
        ...
        toast.error("Failed to upload the docment");
      }
    });

    await Promise.allSettled(uploadPromises);

    //upload the local state with successfully uploaded documents
    if(uploadedDocuments.length > 0){
      ...
      toast.success(`${uploadedDocuments.length} files(s) uploaded`)
    }
  };
  ...
  const handleDocumentDelete = async (documentId: string) => {
    ...
    try{
      ...
      toast.success("Doocument deleted successfully");
    } catch (err) {
  ...
  const selectedDocument = selectedDocumentId
    ? data.documents.find((doc: { id?: string }) => doc?.id === selectedDocumentId)
    : null;
